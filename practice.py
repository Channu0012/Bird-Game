...
class Bird:
    ...
    def draw(self, canvas):
        ...
        # wing feather shadow
        canvas.create_oval(x - 12, y - 8, x + 20, y + 8, fill="#f4b942", outline="", width=0)

        # belly
        canvas.create_oval(x - 7, y - 2, x + 12, y + 16, fill="#fff4c2", outline="")

        # beak
        canvas.create_polygon(x + 18, y + 1, x + 34, y + 6, x + 19, y + 12, fill="#ff8f1f", outline="#dd6a0a", width=2)

        # eye
        canvas.create_oval(x + 9, y - 4, x + 17, y + 4, fill="white", outline="")
        canvas.create_oval(x + 12, y - 2, x + 15, y + 1, fill="#111827", outline="")

        # eye highlight
        canvas.create_oval(x + 13, y - 2, x + 14, y - 1, fill="white", outline="")

        # tail
        canvas.create_polygon(x - 18, y - 2, x - 32, y - 12, x - 28, y + 8, fill="#f59e0b", outline="")

        # trail when flying up
        if self.velocity < 0:
            canvas.create_line(x - 22, y + 4, x - 44, y + 14, fill="#ffe082", width=3)
            canvas.create_line(x - 35, y + 12, x - 55, y + 18, fill="#fff7cc", width=2)
...
class Pipe:
    def __init__(self, x):
        ...
        self.gap_top = random.randint(140, 260)
    ...
    def draw(self, canvas):
        ...
        canvas.create_rectangle(self.x - 8, top_h - 22, self.x + self.width + 8, top_h, fill="#70e2a3", outline="")
        canvas.create_rectangle(self.x + 10, top_h - 14, self.x + self.width - 10, top_h + 8, fill="#22c55e", outline="")

        # bottom pipe body
        canvas.create_rectangle(self.x, bottom_y, self.x + self.width, HEIGHT - 40, fill="#4ade80", outline="#15803d", width=4)
        canvas.create_rectangle(self.x - 8, bottom_y, self.x + self.width + 8, bottom_y + 22, fill="#70e2a3", outline="")
        canvas.create_rectangle(self.x + 10, bottom_y - 10, self.x + self.width - 10, bottom_y + 8, fill="#22c55e", outline="")
...
class FlappyBirdGame:
    ...
    def draw_background(self):
        ...
        # distant mountains
        self.canvas.create_polygon(0, 340, 120, 200, 240, 300, 380, 180, 540, 320, 660, 210, 820, 280, 900, 340, fill="#1e293b", outline="")
        self.canvas.create_polygon(0, 360, 150, 230, 290, 330, 470, 220, 640, 330, 760, 230, 900, 360, fill="#334155", outline="")

        # sun glow
        self.canvas.create_oval(670, 55, 845, 230, fill="#fbbf24", outline="", stipple="gray25")
        self.canvas.create_oval(700, 90, 815, 205, fill="#fde68a", outline="")

        # clouds
        cloud_colors = ["#e2e8f0", "#dbeafe", "#bfdbfe"]
        for i, color in enumerate(cloud_colors):
            x_base = 90 + i * 210
            self.canvas.create_oval(x_base, 100, x_base + 60, 150, fill=color, outline="")
            self.canvas.create_oval(x_base + 30, 80, x_base + 110, 160, fill=color, outline="")
            self.canvas.create_oval(x_base + 80, 95, x_base + 150, 155, fill=color, outline="")

        # small forest silhouette
        for x in range(-30, WIDTH + 50, 45):
            self.canvas.create_rectangle(x, 390, x + 20, 460, fill="#0f766e", outline="")
            self.canvas.create_polygon(x - 10, 390, x + 10, 330, x + 30, 390, fill="#0b5d5d", outline="")

        # flying birds in sky
        for x in [120, 250, 460, 610, 780]:
    ...
    def draw_ground(self):
        ...
        self.canvas.create_rectangle(0, GROUND_Y + 22, WIDTH, HEIGHT, fill="#7c4b36", outline="")
    ...
    def draw_start_overlay(self):
        self.canvas.create_rectangle(140, 145, WIDTH - 140, HEIGHT - 130, fill="#0f172a", outline="#8b5cf6", width=4)
        self.canvas.create_text(WIDTH // 2, 230, text="Crazy Bird", fill="#f8fafc", font=("Arial", 36, "bold"))
        self.canvas.create_text(WIDTH // 2, 285, text="Press Space or Click", fill="#e2e8f0", font=("Arial", 28, "bold"))
        self.canvas.create_text(WIDTH // 2, 325, text="to start flying!", fill="#93c5fd", font=("Arial", 18))
